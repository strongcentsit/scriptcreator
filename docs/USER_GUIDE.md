...
This guide covers everything needed to run and configure Script Creator, even if you've never touched the codebase before. It documents what exists today, including a few rough edges (dead code, hardcoded paths) so you don't get tripped up by them.
...
src/main/java/
  org/example/
    Main1.java              Entry point — Setup Script Generator (the real one)
    Main.java                Empty/unused — do not use
  promo/
    Main.java                Entry point — Promo Queue Generator
  ...
  config/
    ...
    GenerationMode.java            Unused/reserved enum — ignore
  metadata/
  ...
  util/
    ...
    TriggerUtils.java          Unused — superseded by MetadataUtils
    CsvToExcelConverter.java   Standalone CSV → Excel helper (see Feature 3)
```

## Running the tool

There's no single CLI entry point — this is a source project with three independent `main` methods. Pick the one for the feature you need and run it with the project root as the working directory (so relative paths like `input/...` resolve correctly).

**Option A — from an IDE (recommended)**: open the project, right-click the target class, and Run:
- [`org.example.Main1`](../src/main/java/org/example/Main1.java) for the Setup Script Generator
- [`promo.Main`](../src/main/java/promo/Main.java) for the Promo Queue Generator

Set the working directory to the project root in your run configuration if your IDE defaults elsewhere.

**Option B — from the command line**, add the exec plugin to `pom.xml` once:
```xml
<build>
  <plugins>
    <plugin>
...
then run:
```bash
mvn compile exec:java -Dexec.mainClass=org.example.Main1
```
(swap the class name for `promo.Main` as needed). Run this from the project root.

None of the entry points take command-line arguments — all file paths are hardcoded in each `main` method (see each feature section below for the exact paths).

## Feature 1: Setup Script Generator

**Entry point:** [`org.example.Main1`](../src/main/java/org/example/Main1.java)
...
**Entry point:** [`promo.Main`](../src/main/java/promo/Main.java)
...
**Class:** [`util.CsvToExcelConverter`](../src/main/java/util/CsvToExcelConverter.java) (has its own `main` method, but is **not** called by either of the two pipelines above — it's a standalone helper)
...
All configuration lives in source code under `src/main/java/config/` — there is no external config file. To change behavior, edit the relevant class and rebuild.

### `SetupRegistry`
[`src/main/java/config/SetupRegistry.java`](../src/main/java/config/SetupRegistry.java) — the catalog of every setup the tool knows about, plus the switch that decides which ones actually run (see [Choosing which setups run](#choosing-which-setups-run)).
...
[`src/main/java/config/ColumnOverrideConfig.java`](../src/main/java/config/ColumnOverrideConfig.java) — column names that get their value replaced automatically on every insert/update, across every setup, regardless of what's in the source data. Mostly audit columns:
...
[`src/main/java/config/GlobalBusinessKeyConfig.java`](../src/main/java/config/GlobalBusinessKeyConfig.java) — registers business keys for tables that aren't a setup's own main table, but that other setups' data has foreign keys into (so those FK values can be remapped from source IDs to target IDs). Currently only `RES_ADV_NOTE_TYPE` (business key: `DESCRIPTION`) is registered.

### `TableNameMapper`
[`src/main/java/config/TableNameMapper.java`](../src/main/java/config/TableNameMapper.java) — maps a truncated Excel sheet name to its real table name, for tables whose name exceeds Excel's 31-character sheet-name limit. Register a new mapping here if you add a long-named table and the automatic fuzzy-matching (see [Input files](#input-files)) doesn't resolve it correctly.

### `SetupConfigConstants`
[`src/main/java/config/SetupConfigConstants.java`](../src/main/java/config/SetupConfigConstants.java) — `PK_SEQUENCE_OFFSET` (gap above the max target ID used when auto-computing a new starting ID, default 100) and `MAX_SQL_IDENTIFIER_LENGTH` (30, Oracle's identifier length limit, used when naming `B_<table>` backup tables).
...
For full detail on any setup's conditions, business keys, and exclusions, read its `Builder(...)` block in [`SetupRegistry.java`](../src/main/java/config/SetupRegistry.java) — the setup names in that file match this table exactly.

## Choosing which setups run

Open [`SetupRegistry.java`](../src/main/java/config/SetupRegistry.java) and find:
...
- Rebuild (`mvn compile`) after editing, then re-run `org.example.Main1`.

## Adding a new setup

1. Make sure the table(s) involved are present in `input/pk.xlsx`, `input/fk.xlsx`, and `input/columns.xlsx`.
2. Open [`SetupRegistry.java`](../src/main/java/config/SetupRegistry.java) and add a new `SETUP_CATALOG.put(...)` entry (or equivalent registration call) using `new SetupConfig.Builder("Your Setup Name", "MAIN_TABLE")` with whatever options from the [configuration reference](#configuration-reference) you need.
3. Add the new setup's name to `ACTIVE_SETUPS` so it actually gets generated.
4. If the table has a long name that would get truncated as an Excel sheet name, register it in [`TableNameMapper`](../src/main/java/config/TableNameMapper.java).
5. Rebuild and run.
...
- **Class won't run / "no main manifest attribute"** — this project has no packaged JAR; run classes from an IDE, or add the `exec-maven-plugin` as shown in [Running the tool](#running-the-tool).

## Known limitations / dead code

Documented here so they're not mistaken for bugs in behavior you're relying on:

- `org.example.Main` has an empty body — it does nothing. Use `org.example.Main1` for the setup generator.
...
- `TriggerUtils` is unused; trigger loading actually happens inside `MetadataUtils`.
...
- `promo.Main` hardcodes `PromoType.ACCOM_SUPPLIERS` — there's no way to generate for another promo type without editing the source, and only `ACCOM_SUPPLIERS` has its workflow steps configured in `PromoConfigRegistry` anyway.
